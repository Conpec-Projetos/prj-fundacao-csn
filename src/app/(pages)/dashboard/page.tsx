...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
 ...
 function somarDadosEstados(array: dadosEstados[], projRepetidos: Map<string, ProjetoInfo>): dadosEstados {
 ...
 async function buscarProjetosEmLote(projetoIds: string[]): Promise<Record<string, any>> {
 ...
 if(id.idEscolhido == 'projetos'){ // ou seja so tem forms de cadastro obrigatoriamente
 ...
 correcao.beneficiariosDiretos = data?.beneficiariosDireto ?? data?.beneficiariosDiretos ?? 0;
 correcao.beneficiariosIndiretos = data?.beneficiariosIndireto ?? data?.beneficiariosIndiretos ?? 0;
 correcao.ods = data?.ods ?? [];
 return correcao;
 }
 ...
 const projetosRepetidosData = await buscarProjetosEmLote(projetosRepetidosIds);
 ...
 dadosSomados.projetosODS = dadosSomados.projetosODS.map((valor, index) => {
 // Se este ODS está no projeto, subtrai 1 para cada repetição
 const shouldSubtract = correcao.ods.includes(index + 1) || correcao.ods.includes(index);
 return shouldSubtract ? Math.max(0, valor - (info.vezes - 1)) : valor;
 });
 }
 }
 }
 ...
 if (formsSnapshot.exists()) { // verifica se o forms de acompanhamento existe, pois se existir os dados vem dele
